...
class StagesController extends Controller
{
    ...
    {   
	    ...
	    $data['pasosEjecutables']=$pasosEjecutables;
    ...
    public function inbox(Request $request)
    {
        ...
        $etapas = Etapa::select('etapa.*')->where('etapa.usuario_id', Auth::user()->id)->where('etapa.pendiente', 1)
    ...
    public function ejecutar_form(Request $request, $etapa_id, $secuencia)
    {
        ...
        $validations = [];
        $tipos_no_serializados = array("checkbox","radio","comunas","ci");
        if ($modo == 'edicion') {
            ...
            foreach ($formulario->Campos as $c) {
                ...
                if ($c->isEditableWithCurrentPOST($request, $etapa_id)) {
                    ...
                    if($c->tipo=='comunas'){
                        ...
                        $ciudadano = $request->input($c->nombre);
                        $ciudadano['cedula'] = $request->input($c->nombre.'_cedula_'.$c->id);
                        $ciudadano['nombres'] = $request->input($c->nombre.'_nombres_'.$c->id);
                        $ciudadano['apellidos'] = $request->input($c->nombre.'_apellidos_'.$c->id);
                        $ciudadano['fechaNacimiento'] = $request->input($c->nombre.'_fechaNacimiento_'.$c->id);
                        $ciudadano['nacionalidad'] = $request->input($c->nombre.'_nacionalidad_'.$c->id);
                        $dato->valor = $ciudadano;
                    }
