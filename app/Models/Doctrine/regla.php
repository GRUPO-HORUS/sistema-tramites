...
class Regla
{
    ...
    public function getExpresionParaEvaluar($etapa_id, $ev = FALSE)
    {
        ...
        $new_regla = preg_replace_callback('/@@(\w+)((->\w+|\[\w+\])*)/', function ($match) use ($etapa_id, $ev) {
            ...
            if ($dato){
                ...
                try {
                    ...
                    $valor_dato = 'json_decode(\'' . json_encode(str_replace("'","",$dato_almacenado)) . '\')';
                } catch (Exception $e) {
                    $dato_almacenado = '';
        ...
        $new_regla = preg_replace_callback('/@!(\w+)/', function ($match) use ($etapa_id) {
			...
			else if ($nombre_dato == 'etapa_id')
                return $etapa_id;
			else if( strncmp($nombre_dato , 'certificado_validador_url_' , 26)==0){
                $file = Doctrine::getTable('File')->findOneByTipoAndFilename('documento', substr($nombre_dato,26).'.pdf');
				return url('validador/documento?id=' . $file->id . '&key=' . $file->llave_copia);
            }
			else if( strncmp($nombre_dato , 'certificado_id_' , 15)==0){
                $file = Doctrine::getTable('File')->findOneByTipoAndFilename('documento', substr($nombre_dato,15).'.pdf');
				return $file->id;
            }
			else if( strncmp($nombre_dato , 'certificado_key_' , 16)==0){
                $file = Doctrine::getTable('File')->findOneByTipoAndFilename('documento', substr($nombre_dato,16).'.pdf');
				return $file->llave_copia;
            }
        }, $new_regla);
        ...
        return str_replace('.pdf','',$new_regla);
    }
    ...
    public function getExpresionParaOutput($etapa_id, $evaluar = false)
    {
        ...
        $new_regla = preg_replace_callback('/@#(\w+)/', function ($match) use ($etapa_id) {
            ...
            $etapa = Doctrine::getTable('Etapa')->find($etapa_id);
	    //Log::debug("############# etapa: " . $etapa_id);
	    //Log::debug("############# tramite: " . $etapa->Tramite->id);
            $dato = Doctrine::getTable('DatoSeguimiento')->findGlobalByNombreAndProceso($nombre_dato, $etapa->Tramite->id);
	    //Log::debug("############# 2. dato: " . json_encode($dato));
            $valor_dato = json_encode($dato);
	    //Log::debug("############# 2. dato: " . $dato);
        ...
        $new_regla = preg_replace_callback('/@!(\w+)/', function ($match) use ($etapa_id) {
            ...
            if ($nombre_dato == 'rut')
                ...
                return $etapa_id;
			else if( strncmp($nombre_dato , 'certificado_validador_url_encode_' , 33)==0){
                $file = Doctrine::getTable('File')->findOneByFilename(substr($nombre_dato,33).'.pdf');
				return urlencode(url('validador/documento?id=' . $file->id . '&key=' . $file->llave_copia));
            }
			else if( strncmp($nombre_dato , 'certificado_validador_url_' , 26)==0){
                $file = Doctrine::getTable('File')->findOneByFilename(substr($nombre_dato,26).'.pdf');
				return url('validador/documento?id=' . $file->id . '&key=' . $file->llave_copia);
            }
			else if( strncmp($nombre_dato , 'certificado_id_' , 15)==0){
                $file = Doctrine::getTable('File')->findOneByFilename(substr($nombre_dato,15).'.pdf');
				return $file->id;
            }
			else if( strncmp($nombre_dato , 'certificado_key_' , 16)==0){
                $file = Doctrine::getTable('File')->findOneByFilename(substr($nombre_dato,16).'.pdf');
				return $file->llave_copia;
            }
        }, $new_regla);
