...
class CampoDocumentoCarnetQR extends Campo
{
    ...
    private function displayFirmador($modo, $dato, $etapa_id)
    {
        ...
        $display .= '<p><a class="btn btn-info" href="' . url('documentos/get/0/' . $dato->valor) . '?id=' . $file->id . '&amp;token=' . $file->llave . '"><i class="icon-search icon-white"></i> Previsualizar el documento</a></p>';
      ...
      //  $isMac = stripos($_SERVER['HTTP_USER_AGENT'], 'macintosh') !== false;
        ...
        $display .= '
            ...
            <applet code="' . ($isMac ? 'cl.agile.pdf.applet.SignerAppletMinSegPressMAC' : 'cl.agile.pdf.applet.SignerAppletMinSegPress') . '" width="350" height="25" name="SignerApplet">
                <param name="jnlp_href" value="' . url('') . '/assets/applets/signer/' . ($isMac ? 'SignerApplet_0_7_mac.jnlp' : 'SignerApplet_0_9_win.jnlp') . '" />
                ...
                <param name="urlBaseGet" value="' . url('documentos/firma_get') . '" />
                <param name="urlBasePost" value="' . url('documentos/firma_post') . '" />
        ...
        $display .= '            
            <script>                                   
                function deshabilitarFirma(){
                    $("#btnFirmar").hide();
                }
            </script>
            <div id="firmaDiv">
                <div>
                    <a class="btn btn-success" onclick="deshabilitarFirma()" id="btnFirmar"
                        href="pyfirma://sign?urlDoc='.url('documentos/firma_get').'&llaveApp=rXdk5TGsd&token='.$file->llave_firma.'&id='.$file->id.'&comentario=Comentario&urlDest='.url('documentos/firma_post').'&id_session='.Auth::user()->id.'&tokenlaravel='.csrf_token().'">
                        <i class="icon-pencil icon-white"></i>Firmar Documento</a>
                </div>
            </div>';
