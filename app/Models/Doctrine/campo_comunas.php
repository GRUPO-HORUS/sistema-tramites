...
class CampoComunas extends Campo
{
    ...
    protected function display($modo, $dato)
    {
        ...
        $display .= '<option value="">Seleccione Departamento</option>';
        $display .= '</select>';
        $display .= '<br />';
        ...
        $display .= '<option value="">Seleccione Municipio</option>';
        $display .= '</select>';
        ...
        $display .= '
            <script>
                $(document).ready(function(){
                    ...
                    $("#regiones_'.$this->id.'").chosen({placeholder_text: "Seleccione Departamento"});
                    $("#comunas_'.$this->id.'").chosen({placeholder_text: "Seleccione Municipio"});

                    updateRegiones();
                    
                    function updateRegiones(){
                        $.getJSON("/identidadelectronica-backend/api/departamento",function(data){
                            
                            var regiones_obj = $("#regiones_'.$this->id.'");
                            regiones_obj.empty();
                            $.each(data, function(idx, el){
                                regiones_obj.append("<option data-id=\""+el.valor+"\" value=\""+el.etiqueta+"\">"+el.etiqueta+"</option>");
                            });
                    ...
                    function updateComunas(regionId){
                        ...
                        $.getJSON("/identidadelectronica-backend/api/municipio/"+regionId,function(data){
                            if(data){
                                $.each(data, function(idx, el){
                                    var op = el[opcion];
                                    comunas_obj.append("<option data-id=\""+el.valor+"\" value=\""+el.valor+"\" >"+el.etiqueta+"</option>"); 
                                });
                            }
		    ...
		    window.updateComunas=updateComunas;
                });
